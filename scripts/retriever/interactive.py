...
parser.add_argument('--adaptive-model', type=str, default=None)
args = parser.parse_args()

logger.info('Initializing ranker...')
ranker = retriever.get_class('tfidf')(tfidf_path=args.model, adaptive_model_path=args.adaptive_model)
...
def calculate_top_n_ratio(top_n=3):
    ...
    for i in df.index:
        ...
        nb_documents_sum += len(doc_names)
    ...
    return (nb_included_in_top_n, nb_included_in_top_n / df.shape[0],
            positions_sum / nb_included_in_top_n, nb_documents_sum / df.shape[0])
